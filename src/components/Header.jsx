...
import { GithubIcon, LinkedInIcon, FacebookIcon, TiktokIcon } from './icons'; 
...
const Header = ({ onAboutClick }) => {
  ...
  onClick={toggleMenu}
>
  <span className={`block w-8 h-1 bg-[#1B1B1B] rounded-[10px] transition-all duration-300 ease-in-out origin-center
...
>
        {/* Social Links */}
        <nav
  className="social-nav flex items-center justify-center gap-4 flex-wrap"
>
        ...
        <nav className="main-nav flex flex-col lg:flex-row items-center justify-center lg:mb-0">
          <NavLink
            ...
            className={({ isActive }) =>
              ...
              py-2 px-4 rounded-lg hover:bg-black/5 transition-colors duration-300` // Changed text-[18px] to text-base
            }
          >
            Home
          </NavLink>
          <a
            ...
            className="text-base font-medium mx-2 text-black relative after:content-[''] after:absolute after:left-0 after:bottom-[-2px] after:h-[1px] after:w-0 after:bg-current after:transition-all after:duration-300 after:ease-in hover:after:w-full py-2 px-4 rounded-lg hover:bg-black/5 transition-colors duration-300" // Changed text-[18px] to text-base
          >
            About
          </a>
          <a
